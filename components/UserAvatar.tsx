...
export const UserAvatar: React.FC<UserAvatarProps> = ({ userId, className = '' }) => {
  ...
  // If className has width/height, use them, otherwise default to 40px
  // Note: The parent component (AuthWidget) is now passing specific w/h classes.
  // We apply style width/height only if not overridden by className helper logic, 
  // but simpler is to rely on className for dimensions if provided.
  
  const hasCustomSize = className.includes('w-') && className.includes('h-');
  const style = hasCustomSize ? { backgroundColor: bgColor } : { backgroundColor: bgColor, width: '40px', height: '40px' };
